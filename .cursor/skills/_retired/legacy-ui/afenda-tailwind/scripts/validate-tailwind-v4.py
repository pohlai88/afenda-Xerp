...
Usage (from repo root):
  python .cursor/skills/afenda-tailwind/scripts/validate-tailwind-v4.py --root .
...
def check_tailwind_version(root: Path, result: ValidationResult) -> bool:
    ...
    pkg_paths = [find_package_json(root)]

    # Monorepo: also check common app/package manifests when root has no direct dep
    for rel in ('apps/erp/package.json', 'packages/ui/package.json', 'apps/storybook/package.json'):
        candidate = root / rel
        if candidate.exists():
            pkg_paths.append(candidate)

    pkg_path = next((p for p in pkg_paths if p and p.exists()), None)
    if not pkg_path:
    ...
    for candidate in pkg_paths:
        ...
        try:
            ...
            result.add_issue('error', str(candidate.relative_to(root)), 'Invalid JSON in package.json')
            return False
        ...
        resolved_from = str(candidate.relative_to(root))
        break

    if not resolved_version:
        result.add_issue('error', 'package.json', 'tailwindcss not found in workspace dependencies',
                        fix='pnpm add -D tailwindcss@latest @tailwindcss/postcss -F @afenda/erp')
        return False

    result.tailwind_version = resolved_version
    if resolved_from:
        result.add_passed(f'Tailwind version from {resolved_from}: {resolved_version}')

    version_match = re.search(r'(\d+)\.', resolved_version)
    if version_match:
        major = int(version_match.group(1))
        if major < 4:
            result.add_issue('error', resolved_from or 'package.json',
                           f'Tailwind v3 detected ({resolved_version}). Must use v4+',
                           fix='pnpm remove tailwindcss && pnpm add -D tailwindcss@latest @tailwindcss/postcss')
            return False
        return True

    result.add_issue('warning', resolved_from or 'package.json',
                    f'Could not parse Tailwind version: {resolved_version}')
    return True
...
def find_css_files(root: Path) -> list[Path]:
    """Find all CSS files in the project, skipping node_modules and build output."""
    css_files: list[Path] = []
    exclude_dirs = {'node_modules', 'dist', 'build', '.next', '.output', 'out'}
    search_dirs = ['src', 'app', 'apps', 'packages', 'styles', 'css', 'public', '.storybook']

    def walk_css(base: Path) -> None:
        try:
...
def check_css_files(root: Path, result: ValidationResult):
    ...
    for css_file in css_files:
        ...
        if '@afenda/ui/styles/globals.css' in content or '@afenda/ui/afenda-ui.css' in content:
            has_extension_stylesheet = True
            result.add_passed(
                f'Package stylesheet imports Afenda UI bundle in {rel_path}'
            )
        ...
        if '@theme' in content:
            theme_match = re.search(r'@theme\s*(?:inline)?\s*\{([^}]*)\}', content, re.DOTALL)
            if theme_match:
...
def print_report(result: ValidationResult, verbose: bool = False):
    ...
    print("\n" + "=" * 50)
    print("  Afenda Tailwind v4 Validation Report")
    print("=" * 50 + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Validate Tailwind CSS v4 configuration (Afenda monorepo)'
    )
    parser.add_argument(
        '--root', '-r',
        type=str,
