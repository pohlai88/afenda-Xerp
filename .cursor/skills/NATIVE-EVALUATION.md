...
| `afenda-editorial-lab` | **keep** | 2026-07-03 — Afenda editorial taste constitution; Swiss/Verdant/Presentation Lab SSOT router |
| `afenda-editorial-compose` | **keep** | 2026-07-03 — design-plan-first editorial implementation; mandatory Storybook preview URL |
| `afenda-presentation-promotion` | **keep** | 2026-07-03 — lab → auth-shell → ERP promotion; explicit invoke only |
| `afenda-editorial-bundle` | **keep** | 2026-07-03 — editorial preflight bundle; stacks on coding-consistency-bundle; **auto-route** (no `disable-model-invocation`; `paths` + trigger phrases) |

**Ghost catalog note:** `docs-editorial-design` remains a planned Fumadocs prose skill — **not** ERP editorial authority. ERP editorial = `afenda-editorial-lab` + `afenda-editorial-compose`.

**Do not install for Afenda editorial:** raw `anthropics/frontend-design`, `creative-frontend-aesthetics`, shadcn `/iui` as aesthetic source of truth.
...
| Bundle | `*-bundle` | implementer globs | `true` (exception: `afenda-editorial-bundle` omits — auto-route via `paths` + trigger phrases) |
