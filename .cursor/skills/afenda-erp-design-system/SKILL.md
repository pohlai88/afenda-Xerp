...
1. Start with the repo contract.
   ...
   - Prefer the repo's PAS-006 and package-local docs over generic OSS advice.

2. Identify the layer.
   - Strategy or prompt design: no file edits unless requested.
   - Theme/CSS: `packages/shadcn-studio/src/styles/**`, package CSS dist, ERP/Storybook composition entries.
   - Primitive/block/product: `packages/shadcn-studio/src/components-ui/**`, `components-layouts/**`, `components-auth-shell/**`, `components-quarantine/**`, meta registries.
   - ERP surface: `apps/erp/**` consumes the public package barrel and metadata/surface templates; it must not redefine design-system truth.
   - Storybook evidence: `apps/storybook/**` or package stories document states, density, a11y, and interaction proof.

3. Discover before creating.
   - Search existing primitives, blocks, registries, exports, stories, and package docs.
   - Check `components.json`, `package.json` exports, CSS import paths, theme manifest, and current gates before assuming older instructions are still accurate.
   - Query Storybook MCP docs first when it is available for UI component work.

4. Choose the smallest durable move.
   - Reuse an existing shadcn primitive or block before adding a new abstraction.
...
5. Build through the manufacturing chain.
   ...
   - ERP wiring imports from `@afenda/shadcn-studio`, not package internals or quarantine.

6. Verify with narrow gates.
   - Theme/CSS: sync package CSS dist and run package CSS sync checks.
   - Studio package: typecheck, tests, import-zone, quarantine, metadata, slot marker, and block gates as relevant.
   - ERP surface: ERP typecheck/build plus metadata consumer gates.
   - Storybook work: generate stories and run Storybook tests/a11y when the surface needs visual proof.

## Design Doctrine

Use shadcn/ui as open code, not a black-box component library. Afenda owns the resulting design system through `@afenda/shadcn-studio` contracts, registries, tokens, and tests.

Use Tailwind v4 as the design-token runtime. Theme variables and semantic CSS custom properties should drive utility classes; app CSS entries should mostly compose imports and `@source` directives.
...
- Importing `@afenda/kernel`, auth/database/runtime packages, or ERP routes into `@afenda/shadcn-studio`.
- Wiring `components-quarantine/**` or raw vendor output to ERP or Storybook production paths.
...
- Claiming "latest" external docs or library behavior without checking current docs.

## Completion Checklist

- Authority loaded: PAS-006 family and package-local docs for repo work.
- Existing inventory checked before creating new primitives, blocks, tokens, or stories.
- KISS/DRY decision recorded: reuse, extend, or create with reason.
- No parallel source of truth introduced.
- Required gates run or explicitly reported as not run.
