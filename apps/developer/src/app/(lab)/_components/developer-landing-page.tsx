...
const featureLanes = [
  {
    code: "01",
    detail:
      "Tokens are inspected as operating constraints: color, radius, spacing, shadow, typography, and motion.",
    id: "token-forensics",
  ...
  },
  {
    code: "02",
    detail:
      "Imported blocks are held in proof state until structure, naming, accessibility, and ERP fit are accepted.",
    id: "block-quarantine",
  ...
  },
  {
    code: "03",
    detail:
      "Each surface receives a final acceptance read before it becomes part of the presentation runtime.",
    id: "acceptance-gate",
...
export function DeveloperLandingPage() {
  return (
    <main className="dark bg-[#030303] text-neutral-200 selection:bg-violet-950/50 selection:text-neutral-50">
      ...
      <section
        className="relative isolate overflow-hidden border-neutral-900 border-t bg-[#040404] px-5 py-20 sm:px-8 lg:px-12"
        id="features"
      >
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 -z-20 opacity-[0.016] [background-image:linear-gradient(to_right,#ffffff_1px,transparent_1px),linear-gradient(to_bottom,#ffffff_1px,transparent_1px)] [background-size:32px_32px]"
        />
        <div
          aria-hidden="true"
          className="pointer-events-none absolute top-12 right-[6vw] -z-10 hidden select-none font-black text-[clamp(7rem,15vw,14rem)] text-neutral-100/[0.055] leading-none tracking-normal [font-family:Arial_Black,Arial,Helvetica,sans-serif] md:block"
        >
          FEATURES
        </div>

        <div className="mx-auto grid w-full max-w-[1480px] gap-12 lg:grid-cols-[0.72fr_1.28fr]">
          <div className="max-w-md">
            <p className="font-medium text-[0.56rem] text-neutral-500 uppercase tracking-[0.42em]">
              Remaining System
            </p>
            <h2 className="mt-5 max-w-sm font-serif text-3xl text-neutral-200 leading-tight md:text-4xl">
              Features as proof rails, not product noise.
            </h2>
            <p className="mt-6 max-w-sm border-neutral-800 border-l pl-4 text-[0.68rem] text-neutral-600 uppercase leading-6 tracking-[0.18em]">
              The page continues as a restrained verification sequence after the
              theatre capture.
            </p>
          </div>

          <div className="border-neutral-800/80 border-y">
            {featureLanes.map((feature) => (
              <article
                className="grid gap-5 border-neutral-900 border-b py-7 last:border-b-0 md:grid-cols-[5rem_minmax(0,0.7fr)_minmax(0,1fr)] md:items-center"
                key={feature.id}
              >
                <p className="font-serif text-4xl text-[#c3a35f]/80 leading-none">
                  {feature.code}
                </p>
                <h3 className="text-neutral-300 text-sm uppercase tracking-[0.28em]">
                  {feature.title}
                </h3>
                <p className="max-w-xl text-[0.78rem] text-neutral-500 leading-6">
                  {feature.detail}
                </p>
              </article>
            ))}
