...
const PROOF_SOURCE_FILE_PATTERN = /\.(ts|tsx)$/;
...
const v2ProofAppRoot = path.join(appRoot, "design-system", "v2-proof");
const v2ProofLibRoot = path.join(developerRoot, "src", "lib", "v2-proof");
...
const resolveRouteFilesystemDir = (route) => {
  if (route.routeProfile === "consumer-proof") {
    return path.join(appRoot, ...route.href.split("/").filter(Boolean));
  }
...
const collectProofSourceFiles = (directory) => {
  const files = [];
  ...
  for (const entry of readdirSync(directory)) {
    ...
    if (
      PROOF_SOURCE_FILE_PATTERN.test(entry) &&
...
const activeRoutes = routeRegistryEntryBlocks
  .map((entryBlock) => ({
    ...
    importLaw:
      getRegistryStringField(entryBlock, "importLaw") ?? "lab-runtime-ceiling",
    kind: getRegistryStringField(entryBlock, "kind") ?? "",
    querySeam:
      getRegistryStringField(entryBlock, "querySeam") ?? "placeholder-only",
    requiresLoadingBoundary:
      getRegistryBooleanField(entryBlock, "requiresLoadingBoundary") ?? true,
    routeId: getRegistryStringField(entryBlock, "routeId") ?? "",
    routePath: getRegistryStringField(entryBlock, "routePath") ?? "",
    routeProfile:
      getRegistryStringField(entryBlock, "routeProfile") ?? "operator-lab",
...
for (const entryBlock of routeRegistryEntryBlocks) {
  ...
  const isConsumerProof = routeProfile === "consumer-proof";
  const isOperatorLab = routeProfile === "operator-lab";
  ...
  if (href !== "/") {
    if (kind === "static-surface" && !isConsumerProof) {
      recordFailure(
        routeSurfaceRegistryPath,
    ...
    if (rendering !== "force-dynamic") {
      recordFailure(
        routeSurfaceRegistryPath,
        `active route ${routeId ?? href} must use force-dynamic rendering`
      );
    }

    if (isOperatorLab && requiresLoadingBoundary !== true) {
      recordFailure(
        routeSurfaceRegistryPath,
        `operator-lab route ${routeId ?? href} must require a loading boundary`
      );
    }

    if (isConsumerProof && requiresLoadingBoundary !== false) {
      recordFailure(
        routeSurfaceRegistryPath,
        `consumer-proof route ${routeId ?? href} must not require a loading boundary`
      );
    }

    if (isConsumerProof && importLaw !== "public-v2-exports-only") {
      recordFailure(
        routeSurfaceRegistryPath,
        `consumer-proof route ${routeId ?? href} must declare importLaw public-v2-exports-only`
      );
    }

    if (isOperatorLab && importLaw !== "lab-runtime-ceiling") {
      recordFailure(
        routeSurfaceRegistryPath,
        `operator-lab route ${routeId ?? href} must declare importLaw lab-runtime-ceiling`
      );
    }
  }
...
for (const route of activeRoutes) {
  const routeDir = resolveRouteFilesystemDir(route);
  const isConsumerProof = route.routeProfile === "consumer-proof";
  const isOperatorLab = route.routeProfile === "operator-lab";
  const actionsDir = isOperatorLab
    ? findNearestRouteActionsDir(routeDir)
    : null;
  const queriesDir = isOperatorLab
    ? findNearestRouteQueriesDir(routeDir)
    : null;
  ...
  if (route.requiresLoadingBoundary && !existsSync(loadingPath)) {
    recordFailure(
      loadingPath,
  ...
  if (isOperatorLab) {
    if (!/export\s+default\s+async\s+function\s+\w+\s*\(/.test(pageSource)) {
      recordFailure(
        pagePath,
        "page.tsx must export a thin async route boundary"
      );
    }
    ...
    if (route.querySeam === "governed-active") {
      ...
      for (const registeredQueryFilePath of registeredQueryFilePaths) {
        const absoluteQueryFilePath = path.join(
          appRoot,
          registeredQueryFilePath
        );
  ...
  if (isConsumerProof) {
    if (!/export\s+default\s+async\s+function\s+\w+\s*\(/.test(pageSource)) {
      recordFailure(
        pagePath,
        "consumer-proof page.tsx must export a thin async route boundary"
      );
    }
    ...
    if (loaderImports.length > 0) {
      recordFailure(
        pagePath,
        "consumer-proof page.tsx must not import lib/lab loaders; use lib/v2-proof fixtures"
      );
    }
    ...
    if (routeLocalImports.length === 0) {
      recordFailure(
        pagePath,
        "consumer-proof page.tsx must render route-local panels imported from ./_components"
      );
    }

    if (
      pageSource.includes("searchParams") &&
      !/searchParams:\s*Promise</.test(pageSource)
    ) {
      recordFailure(
        pagePath,
        "consumer-proof page props must type searchParams as Promise<T> for Next.js 16 App Router compatibility"
      );
    }

    if (
      pageSource.includes("searchParams") &&
      !/await\s+searchParams\b/.test(pageSource)
    ) {
      recordFailure(
        pagePath,
        "consumer-proof page must await searchParams before reading query values"
      );
    }
  }
}
...
const consumerProofRoutes = activeRoutes.filter(
  (route) => route.routeProfile === "consumer-proof"
);

if (
  existsSync(v2ProofAppRoot) &&
  !consumerProofRoutes.some((route) => route.href === "/design-system/v2-proof")
) {
  recordFailure(
    routeSurfaceRegistryPath,
    "filesystem consumer-proof route /design-system/v2-proof must have a matching route-surface-registry entry"
  );
}

for (const proofFilePath of [
  ...collectProofSourceFiles(v2ProofAppRoot),
  ...collectProofSourceFiles(v2ProofLibRoot),
]) {
  const proofSource = readFileSync(proofFilePath, "utf8");
  const relativePath = path.relative(developerRoot, proofFilePath);

  if (V2_INTERNAL_IMPORT_PATTERN.test(proofSource)) {
    recordFailure(
      proofFilePath,
      `${relativePath}: forbidden V2 internal import in consumer-proof profile`
    );
  }

  if (V2_SOURCE_STYLE_IMPORT_PATTERN.test(proofSource)) {
    recordFailure(
      proofFilePath,
      `${relativePath}: forbidden V2 source style path in consumer-proof profile`
    );
  }

  if (FORBIDDEN_V1_PROOF_IMPORT_PATTERN.test(proofSource)) {
    recordFailure(
      proofFilePath,
      `${relativePath}: forbidden legacy v1 import in consumer-proof profile`
    );
  }
}
...
if (existsSync(globalsCssPath)) {
  ...
  if (!globalsCss.includes("@afenda/shadcn-studio-v2/shadcn-default.css")) {
    recordFailure(
      globalsCssPath,
      "globals.css must import @afenda/shadcn-studio-v2/shadcn-default.css for consumer-proof CSS boundary"
    );
  }

  if (globalsCss.includes("packages/shadcn-studio-v2/src/styles")) {
    recordFailure(
      globalsCssPath,
      "globals.css must not import package source style paths"
    );
  }
}

if (failures.length > 0) {
  console.error("Developer app governance check failed.\n");
...
console.log("Developer app governance check passed.");
for (const route of routeRegistryEntryBlocks
  .map((entryBlock) => ({
    href: getRegistryStringField(entryBlock, "href") ?? "",
    routeProfile:
      getRegistryStringField(entryBlock, "routeProfile") ?? "operator-lab",
  ...
  console.log(`- ${route.href} (${route.routeProfile})`);
}
