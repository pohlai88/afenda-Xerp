...
function createFormData(
  payload: ReturnType<typeof buildDefaultUserNotificationsSettings>
): FormData {
...
describe("updateUserNotificationsSettingsAction", () => {
  beforeEach(() => {
  ...
  it("rejects invalid intent", async () => {
    ...
    formData.set(
      "payload",
      JSON.stringify(buildDefaultUserNotificationsSettings())
    );

    const result = await updateUserNotificationsSettingsAction(null, formData);
  ...
  it("rejects unauthenticated requests", async () => {
    ...
    const result = await updateUserNotificationsSettingsAction(
      null,
      createFormData(buildDefaultUserNotificationsSettings())
    );
  ...
  it("persists notifications for the linked actor via upsertUserPreferencesSection", async () => {
    mockResolveActionOperatingContext.mockResolvedValueOnce({
      ok: true,
    ...
    const payload = buildDefaultUserNotificationsSettings();
    payload.dndEnabled = true;
