...
export type UpdateWorkspaceSettingsActionState =
  ServerActionResult<UpdateWorkspaceSettingsData> | null;

function parseWorkspaceActionInput(formData: FormData): unknown {
...
export async function updateWorkspaceSettingsAction(
  ...
  const parsed = parseProtectedActionInput(
    ...
    parseWorkspaceActionInput(formData)
  );
  if (!parsed.ok) {
  ...
  revalidatePath("/system-admin/settings/workspace");
