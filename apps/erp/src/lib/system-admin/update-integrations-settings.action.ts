...
export interface UpdateIntegrationsSettingsData {
  readonly acknowledged: true;
  readonly tenantId: string;
}

export type UpdateIntegrationsSettingsActionState =
  ServerActionResult<UpdateIntegrationsSettingsData> | null;

function parseIntegrationsActionInput(formData: FormData): unknown {
...
export async function updateIntegrationsSettingsAction(
  ...
  const parsed = parseProtectedActionInput(
    updateIntegrationsSettingsInputSchema,
    parseIntegrationsActionInput(formData)
  );
  if (!parsed.ok) {
