...
export async function resolveLegalEntityContext(
  ...
  const entityGroupRow = companyResult.value.entityGroupId
    ? await findEntityGroupById(companyResult.value.entityGroupId, input.db)
    : null;

  const entityGroupBoundaryError = verifyEntityGroupBoundary({
    entityGroupId: legalEntity.entityGroupId,
    entityGroupRow,
    tenantPk: companyResult.value.tenantId,
  });
