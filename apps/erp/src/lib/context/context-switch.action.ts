...
export interface SwitchOperatingContextData {
  readonly operatingContext: PresentationShellOperatingContext;
  readonly workspace: {
...
/** PAS-001A R1b — cookie persistence, session workspace, and audit for context switch. */
export async function switchOperatingContextAction(
  ...
  return serverActionSuccess({
    operatingContext: toPresentationShellOperatingContext(operatingContext),
