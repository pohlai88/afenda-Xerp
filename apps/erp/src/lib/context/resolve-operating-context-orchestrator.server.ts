...
/**
 * Server-side operating context resolver ÔÇö Step 7 ┬º561ÔÇô571.
 * Client-provided slugs/IDs are selection hints only ÔÇö always verified here.
 * Registry: `operating-context-resolver-registry.ts`.
 */
/** Full IS-002 orchestrator — slug/cookie selection path (PAS-001A R1b context switch). */
export async function resolveOperatingContextOrchestrator(
  ...
  const companyRowForGroup = await findCompanyById(legalEntity.companyId, db);
  const { consolidationScope, ownershipInterests } =
    await resolveConsolidationScope({
      tenantEnterpriseId: tenant.tenantId,
      tenantPk: tenantRow.id,
      entityGroup,
      entityGroupPk: companyRowForGroup?.entityGroupId ?? null,
      reportingDate,
      ...(db === undefined ? {} : { db }),
    });
