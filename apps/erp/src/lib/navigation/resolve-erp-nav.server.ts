...
import type { ErpNavGroupWire, ErpNavItemWire } from "@afenda/shadcn-studio";
import {
...
): Promise<readonly ErpNavItemWire[]> {
  const visible: ErpNavItemWire[] = [];
...
/** Serializable operator navigation filtered by read permissions. */
export async function resolveErpNavGroups(
  operatingContext: OperatingContext
): Promise<readonly ErpNavGroupWire[]> {
  const groups: ErpNavGroupWire[] = [];
