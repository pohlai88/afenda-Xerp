...
async function resolveApiRouteActorId(
  context: ApiRequestContext<unknown>
): Promise<string | null> {
  const sessionActorId = resolveActorIdFromSession(context.session);
  if (sessionActorId !== null) {
    return sessionActorId;
  }

  const authActor = await resolveApiRouteAuthActor(context.request.headers);
  if (authActor?.kind === "service") {
    return authActor.identity.authSubjectId;
  }
...
export async function assertRoutePermission(
  ...
  const authorization = await assertAuthorizedApiRoute({
    actorId: await resolveApiRouteActorId(context),
