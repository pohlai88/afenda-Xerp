...
function loadCanvasImage(src: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () =>
      reject(new Error(`Failed to load auth pixel image: ${src}`));
  });
}
...
function buildParticles({
  ...
  const coverScale =
    (canvasAspect > imageAspect
      ...
      : canvasHeight / image.naturalHeight) * 0.74;
...
function drawResolvedImage(
  ...
  const scale =
    (canvasAspect > imageAspect
      ...
      : height / image.naturalHeight) * 0.74;
...
export function AuthPixelCanvas(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    loadCanvasImage(LYNX_IMAGE_SRC)
      ...
      .catch(() => {
        // Decorative only. Keep auth functional even if the pixel asset fails to load.
      });
  ...
  return <canvas className="auth-pixel-canvas" ref={canvasRef} />;
}
