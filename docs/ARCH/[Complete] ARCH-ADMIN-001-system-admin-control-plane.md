...
| **Status** | **Complete — enterprise 9.5 accepted** (29/30; DoD #20 closed 2026-06-25) — Phase A–D + Slices 1–11 delivered (see [`slices/ARCH-ADMIN-001/`](slices/ARCH-ADMIN-001/)) |
| **Date** | 2026-06-25 |
...
| **Clean Core target** | B |
| **Enterprise score target** | 29/30 (9.5+) — **29/30 accepted** (DoD #20 closed 2026-06-25) |
...
```text
1. docs/ARCH/[Complete] ARCH-ADMIN-001-system-admin-control-plane.md          ← this document
...
```text
Phase A–D + Slices 7–11 delivered the system-admin shell, section guards, promoted settings blocks 01–07,
tenant_settings persistence (notifications/workspace/billing/integrations), live Members roster,
Security user MFA actions, settings audit waiver closeout, evidence-sync, TypeScript action dedup,
and Complete promotion with DoD #20 Architecture Authority sign-off (2026-06-25).
ARCH-ADMIN-001 remains the bounded authority so agents do not duplicate auth docs, ship staging UI into ERP,
...
Current estimate:     29/30 (Complete — DoD #20 closed 2026-06-25)
```
...
| 20 | Peer review if required | Architecture Authority sign-off §16 | [x] 2026-06-25 — Slice 11 Complete promotion |

**Promotion:** **Complete — enterprise 9.5 accepted** (29/30). DoD #20 closed 2026-06-25. Operator `pnpm migrate` per environment remains ops debt (§16 criterion 7).
...
- Slices 8–10 delivered with gate evidence ✓
...
> **Closed 2026-06-25.** Filename promoted to `[Complete]` in Slice 11 change set.

| # | Review criterion | Evidence path | Gate | Exit |
| --- | --- | --- | --- | ---: |
| 1 | Seven settings sections live with RSC resolvers | `apps/erp/src/app/(protected)/system-admin/settings/*/page.tsx` | ERP test:run system-admin | 0 |
| 2 | `tenant_settings` persistence (notifications/workspace/billing/integrations) | `packages/database/src/tenant-settings/` | `@afenda/database` test:run | 0 |
| 3 | Members roster + invite resend/revoke audit | `resend-system-admin-invite.action.ts` · `revoke-system-admin-invite.action.ts` | PKG007 registry + parity test | 0 |
| 4 | Settings mutations emit audit (no observability waiver) | `system-admin-observability-registry-parity.test.ts` | `pnpm quality:erp-observability` | 0 |
| 5 | Mutation audit registry complete | `system-admin-mutation-audit.registry.ts` | `pnpm check:system-admin-mutation-audit` | 0 |
| 6 | Documentation authorities aligned | ARCH · matrix · FDR · index | `pnpm check:documentation-drift` | 0 |
| 7 | Operator migration applied per environment | `20260625045902_tenant_settings_integrations` | `pnpm migrate` (per env) | ops |
...
### Slice 9 — TypeScript action dedup + contract cleanup

**Status:** **Delivered — 2026-06-25**  
**Handoff file:** [`docs/ARCH/slices/ARCH-ADMIN-001/slice-09-typescript-action-dedup.md`](slices/ARCH-ADMIN-001/slice-09-typescript-action-dedup.md)  
**Closes:** Shared `executeTenantSettingsSectionUpdate` · deprecated copy export removed · PKG013 `auditWiringPath` for delegated actions

#### Gate evidence (2026-06-25)

```text
pnpm --filter @afenda/erp typecheck                 exit 0
pnpm --filter @afenda/erp test:run system-admin       exit 0 — 104/104
pnpm check:system-admin-mutation-audit              exit 0
pnpm quality:erp-observability                      exit 0
pnpm check:documentation-drift                      exit 0
```
...
### Slice 10 — DoD #20 peer review evidence packet

**Status:** **Delivered — 2026-06-25** — DoD #20 evidence packet complete  
**Handoff file:** [`docs/ARCH/slices/ARCH-ADMIN-001/slice-10-dod20-peer-review-evidence.md`](slices/ARCH-ADMIN-001/slice-10-dod20-peer-review-evidence.md)  
...
| --- | ---: | --- |
| `pnpm --filter @afenda/erp typecheck` | 0 | Pass |
| `pnpm --filter @afenda/erp test:run system-admin` | 0 | Pass — 104/104 |
...
### Slice 11 — Complete promotion (DoD #20 closed)

**Status:** **Complete promotion delivered — 2026-06-25**  
**Handoff file:** [`slice-11-complete-promotion.md`](slices/ARCH-ADMIN-001/slice-11-complete-promotion.md)  
**Prerequisite:** Slice 10 ✓ · DoD #20 Architecture Authority Approved 2026-06-25  
...
- Runtime matrix System Admin row → implemented
- arch-status-index · slice README · inbound links synchronized
...
| [`ARCH-USER-001`](%5BComplete%5D%20ARCH-USER-001-user-settings-self-service.md) | User self-service — profile, personal security, prefs |
