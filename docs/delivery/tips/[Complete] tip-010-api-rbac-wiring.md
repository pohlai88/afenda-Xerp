...
| **Remaining gap** | None — Slice 2 delivered (2026-06-24) |
...
| System-admin audit-events route | `apps/erp/src/app/api/internal/v1/system-admin/audit-events/route.ts` | Yes — Slice 2 |
| System-admin users invite route | `apps/erp/src/app/api/internal/v1/system-admin/users/invite/route.ts` | Yes — Slice 2 |
| System-admin membership role route | `apps/erp/src/app/api/internal/v1/system-admin/memberships/role/route.ts` | Yes — Slice 2 |
| System-admin API contracts | `apps/erp/src/server/api/contracts/system-admin/system-admin.contract.ts` | Yes — Slice 2 |
...
| Full internal v1 matrix closed | `api-route-permissions.test.ts`, `authorize-api-route.test.ts` |
| Cross-company scope mismatch audit | `authorize-api-route.test.ts` — `company_mismatch` + correlationId |
| Entity group vs subsidiary denial at API boundary | `authorize-api-route.test.ts` — entity_group membership + subsidiary outside group |
...
| 8 | All internal v1 routes classified + protected | `api-route-permissions.ts` matrix vs `API_CONTRACTS` | [x] |
| 9 | System-admin API RBAC matrix complete | All system-admin routes + permission keys | [x] |
| 10 | Cross-company scope mismatch denial + audit | Phase 3 gate integration test | [x] |
| 11 | Runtime matrix updated | `docs/architecture/afenda-runtime-truth-matrix.md` | [x] |
| 12 | TIP status index reflects closeout | `docs/delivery/tip-status-index.md` | [x] |
...
```
Handoff from: docs/delivery/tips/[Complete] tip-010-api-rbac-wiring.md
...
**Complete** — `authorizeApiRoute` and `createApiHandler` RBAC integration are delivered for all governed internal v1 routes including workspace dashboard-layout and TIP-013 system-admin APIs. Slice 2 closed the route permission matrix, system-admin RBAC proof, and cross-company `company_mismatch` denial with correlation ID. Full ADR-0001 TIP-010 identity closeout (membership engine, session→context on all surfaces) remains outside this document.
