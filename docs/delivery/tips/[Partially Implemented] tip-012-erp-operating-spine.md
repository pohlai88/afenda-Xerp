...
| **Authority status** | ADR-0001 TIP-012 accepted — full lifecycle including outbox not proven |
| **Runtime evidence** | `packages/kernel/src/context/context-registry.ts`, `apps/erp/src/lib/context/`, `apps/erp/src/server/api/runtime/create-api-handler.ts`, `packages/database/src/audit/` |
| **Status source** | [`afenda-runtime-truth-matrix.md`](../../architecture/afenda-runtime-truth-matrix.md) |
| **Foundation phase** | Phase 2 — Platform Runtime Spine (cross-cutting through Phase 3 closeout) |
| **Remaining gap** | Trigger.dev remote deploy (`pnpm --filter @afenda/execution trigger:deploy` + `TRIGGER_PROJECT_REF`) |
...
- TIP-011 Execution Foundation (partial) — **outbox required for spine closeout**
...
1. [`[Partially Implemented] tip-012-erp-operating-spine.md`](%5BPartially%20Implemented%5D%20tip-012-erp-operating-spine.md) — Runtime evidence + DoD
...
**Partially Implemented** — Operating spine contract, `runProtectedMutation`, dashboard PUT outbox wiring, and lifecycle integration test delivered. **Remaining:** Trigger.dev remote deploy for scheduled outbox publish in production.
