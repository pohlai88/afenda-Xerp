...
| `docs/tip/` directory | Does not exist | **Not created** — delivery authority remains `docs/PAS/CSS-AUTHORITY/SLICE/[status] tip-*.md` |
...
| Action | Result |
| --- | --- |
| Duplicate TIP basenames removed from `docs/PAS/CSS-AUTHORITY/SLICE/` | Only canonical `[status]` file per TIP number (+ † superseded misnumbered evidence) |
