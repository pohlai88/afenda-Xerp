...
| **Business master data authority (TIP-008B)** | `docs/architecture/glossary.md`, `dependency-registry.md` | **partially-implemented** | Customer, Product, Employee, Warehouse, Supplier ownership map — documentation only; PKG-R02–R05 aligned with [`package-registry.md`](package-registry.md) | Runtime contracts + domain packages deferred | Domain TIPs after Phase 1 |
...
| **Database / Drizzle** | `packages/database/` | **partially-implemented** | 141+ TS files; schemas: tenant, company, org, membership, audit, policy, entitlement, **outbox**; migrations; seeds; RLS helpers; tenant RLS completion migration + coverage gate (Slice F) + live apply gate (Slice G) | No accounting tables (correct) | Maintain |
...
| **ERP Application** | `apps/erp/` | **partially-implemented** | Protected dashboard with `WORKSPACE_HOME_COPY` + `resolveWorkspaceDashboardStatusCopy`; `AppShellMain` titleId/contentLabel contract; governed Skeleton/Alert dashboard UX; metadata production page `/metadata-workspace` + `metadata-production-page.test.tsx`; module placeholder UX; System Admin settings form-layout; System Admin audit DataTable (Slice 7); invite wizard with governed `Label`+`RadioGroupItem` role association (Slices 8, 12); context-switch UX (Slice 9); manifest module placeholders; governed API routes; CSP; tests green under full-suite load (Slice 11) | TIP-UI-05 TIP closeout / final acceptance sweep | Maintain |
| **Storybook** | `apps/storybook/` | **implemented** | PKG-021; UI + appshell + metadata-ui stories; test runner | — | Maintain as visual gate |
| **Docs application (`@afenda/docs`)** | `apps/docs/` | **partially-implemented** | Fumadocs MDX + CI gate + porcelain palette (Slice 3.9) + seed content (Slice 4); Afenda Docs reference blocks in `packages/ui/src/components/afenda-docs/` + Storybook (Slice 5); `docs-nav.contract.ts` — `quality:docs` ([`tip-032`](../delivery/tips/[Partially%20Implemented]%20tip-032-implementation-documentation.md) Slices 1–5) | Deploy target (Slice 6); optional MDX block copy into docs app (Slice 5.1) | TIP-032 Slice 6 |
...
| RLS / DB isolation | partial | RLS helpers; tenant RLS artifact gate (`check:database-tenant-rls-coverage`); live apply gate (`check:database-tenant-rls-live`, environment-specific) — **Slices F–G signed off** |
