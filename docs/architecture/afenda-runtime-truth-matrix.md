...
| **Kernel ERP consumer integration (PAS-001A)** | `apps/erp/src/lib/context/` · `apps/erp/src/lib/metadata/` · `@afenda/permissions/src/scope/` | PAS-001A B71–B75 **historical** · B111 skeleton consumer · doctrine Production Candidate | **partially-implemented** | IS-001 live; IS-002/IS-003 **partial** — `TENANT_LIFECYCLE_BRIDGE_WIRING` only (3 entries); no `resolve-operating-context.server.ts`; slim `check:erp-operating-context-spine`; B111 `check:erp-tenant-lifecycle-extension-consumer-attestation`; legacy `check:metadata-context-authorization-bridge` **archived** | PAS-001A-R1 full spine · P006-01 protected shell | See [PAS-001A §1.4](../PAS/KERNEL/PAS-001A-ERP-INTEGRATION-SPINE-STANDARD.md#14-adr-0027-runtime-suspension-2026-06-29) |
| **Kernel ERP domain vocabulary catalog (PAS-001B)** | `packages/kernel/src/erp-domain/` (28 modules) | PAS-001B B76–B106 ✓ · `PKGR01B_ERP_DOMAIN_CATALOG` | **implemented** | 28/28 delivered; unified vocabulary gate green; B106 foundation scaffold (`accounting`, `inventory`) | PAS-001A-R1 metadata consumer on PAS-006 ERP | — |
