...
| **Design System** | `packages/design-system/` | `PKG004_DESIGN` — amber-lane | **partially-implemented** | Token registry, recipes, governance scripts, contrast tests; **no runtime UI** (by design) | PAS/design-authority closeout if promoted to green-lane | Maintain governance gates |
| **UI Primitives** | `packages/ui/src/components/` | `PKG-018 governed-primitives (pas-status-index)` — Not started | **implemented** | 58 component files; 68+ tests; `resolvePrimitiveGovernance()`; stories | ADR-0008 ref-as-prop migration deferred; PAS amber closeout pending | PAS Research + ADR-0008 track |
| **UI Consumption (Governed UI)** | `scripts/governance/ui-guard.mjs`, `docs/governance/governed-ui-policy.md` | `PKG-018 ui-consumption (pas-status-index)` — Not started | **implemented** | Gate D/F scripts; consumer rules in AGENTS.md | PAS slice not reconciled with tip-004 **Complete** archive | Research Slice 1; `pnpm ui:guard:scan` on changes |
| **shadcn/studio (ADR-0017 · PAS-005A)** | `@afenda/shadcn-studio` · `packages/appshell/src/presentation/` · `packages/appshell/src/shadcn-studio-bridge/` · agent skill [`.cursor/skills/afenda-shadcn-components/`](../../.cursor/skills/afenda-shadcn-components/SKILL.md) | `PKGR05A_SHADCN_STUDIO` — **green-lane** (PAS-005A B38–B42p ✓) · [`ADR-0017`](../adr/ADR-0017-shadcn-studio-ui-delivery-acceleration.md) Proposed | **implemented** | MCP product `@afenda/shadcn-studio` (68-row delegating-flip policy registry); wrapper strangler complete; MCP a11y article/footnote contract; bridge exports; `STUDIO-PATTERN-MAP.md`; `afenda-appshell-studio.css` B42l consolidation; `legacy-studio-consumer-scan.test.ts`; B42h deleted legacy `shadcn-studio/` path; B42p zero new flips | Optional ERP feature delegating per block when a11y parity lands | Maintain |
| **CSS Authority (PAS-005)** | `@afenda/css-authority` · `packages/design-system/` token shim | `PKGR05_CSS_AUTHORITY` — **amber-lane** (PAS-005 B26–B37 ✓) | **implemented** | 605-token registry (465 afenda + 44 appshell + 50 auth-editorial + 46 shadcn); `afenda-ui.css` → `afenda-tokens.css` + `afenda-css-authority.css`; consumption R23–R30 + domain-sync + bridge + visual contract + docs pixel baselines pass | Optional enhancements only | Maintain |
| **Enterprise Knowledge (PAS-004C)** | `@afenda/enterprise-knowledge` · `scripts/governance/check-knowledge-*.mts` | `PKGR04_ENTERPRISE_KNOWLEDGE` — PAS-004C (B38–B48 ✓; scorecard 58/58) | **implemented** | JSON authority (`atoms.json` / `edges.json`); semantic model North Star; kernel mapping + consumer proofs (ERP, metadata, docs); B47–B48 consumer projection adoption; `check:knowledge-conformance` + full §13 gate chain | — | Maintain |
| **Accounting Standards (PAS-003)** | `@afenda/accounting-standards` | `PKGR03_ACCOUNTING_STANDARDS` — Production Candidate (B0 ✓) | **partially-implemented** | PAS-003 published; B0 package skeleton + agent skill; Architecture Authority registries | B1 — accounting standard family registry not started | Slice B1 — [`b1-4.1-accounting-standard-family-registry.md`](../PAS/slice/b1-4.1-accounting-standard-family-registry.md) |
| **AppShell** | `packages/appshell/` · `packages/appshell/src/auth-shell/` · `apps/erp/src/app/(auth)/` | `PKG-001 shell-composition (pas-status-index)` **29/30 audit-adjusted** · `PKG-001 manifest-nav (pas-status-index)` Complete · ARCH-AUTH-002 **Complete — 29/30** · ARCH-AUTH-003 **Complete — 29/30** | **implemented** | 93+ `.tsx`; auth-shell lanes + `.af-auth-shell` CSS; tenant brand panel (`logoUrl`/`brandColor`); 18+ package auth-shell tests; `check:auth-shell-boundary` exit 0 (single `(auth)` segment); ERP `(auth)` async layout + `resolveTenantAuthBrand`; canonical URLs `/sign-in`, `/mfa`, … (no `/v2/` prefix); legacy v1 shell decommissioned 2026-06-26; system-admin Appearance settings; first ERP `@afenda/storage` upload route | Live R2 env smoke in deployed preview; registry promotion for PKG-015 ERP consumer | Deployed storage smoke; `foundation-registry-owner` when promoting PKG-015 ERP lane |
| **Tenant settings (appearance)** | `packages/database/src/tenant-settings/` · `apps/erp/src/lib/system-admin/` | ARCH-AUTH-003 · PKG-007 tenant-auth-branding (pas-status-index) | **implemented** | `tenant_settings.appearance` jsonb + governed migration `20260626103428_acoustic_sphinx`; Zod contract + service section; admin save + logo upload API; resolver tests | — | — |
| **Metadata UI** | `packages/metadata-ui/` | `PKG-012 metadata-renderers (pas-status-index)` — Partially Implemented | **implemented** | Renderers + ERP `/metadata-workspace` route; production proof tests | Registry row `PKG012_METADATA_UI` pending | Registry-sync Slice 2 |
| **Metadata Authority** | `packages/ui-composition/` | `PKG-011 metadata-authority (pas-status-index)` — Partially Implemented | **implemented** | Contracts, registries, validation tests (153 tests) | Registry row `PKG011_METADATA` pending | Registry-sync Slice 2 |
...
| **Kernel ERP consumer integration (PAS-001A)** | `apps/erp/src/lib/context/` · `@afenda/permissions/src/scope/` | PAS-001A B71–B75 ✓ · Production Candidate | **implemented** | `CONTEXT_INTEGRATION_WIRING` spine; `check:erp-operating-context-spine`; metadata workspace uses `resolveOperatingContextFromHeaders`; `check:metadata-context-authorization-bridge`; B75 attestation 2026-06-29 | — | Maintain |
| **Kernel ERP domain vocabulary catalog (PAS-001B)** | `packages/kernel/src/erp-domain/erp-domain-layout.contract.ts` · `erp-domain/accounting/` | PAS-001B B76–B78 ✓ · `PKGR01B_ERP_DOMAIN_CATALOG` | **partially-implemented** | 28-slug catalog; 27 catalog-only; `accounting` delivered; `check:erp-domain-layout` 10-point matrix; B78 audit closed 2026-06-28 | B79+ per-module vocabulary | B79 inventory (proposed) |
...
| --- | --- | --- | --- | --- |
| `@afenda/accounting-standards` | `packages/accounting-standards` | Foundation | B0 skeleton (PAS-003) | Yes |
