...
| **Database / Drizzle** | `packages/database/` | [`fdr-003-persistence`](../delivery/FDR/[Partially%20Implemented]%20fdr-003-persistence.md) **27/30 audit-adjusted** (29/30 ceiling) · [`fdr-003-tenant-rls`](../delivery/FDR/[Complete]%20fdr-003-tenant-rls.md) **Complete — 29/30** | **implemented** | 169 tests; 27 migrations; RLS registry + live apply gates | `fdr-003-persistence` peer review pending | Maintain |
| **Auth** | `packages/auth/`, `apps/erp/src/app/api/auth/` | [`fdr-002-auth-disposition`](../delivery/FDR/[Complete]%20fdr-002-auth-disposition.md) **Complete — 29/30** · PKG002_AUTH | **implemented** | Better Auth; session→context bridge on all protected surfaces; 49 PKG tests exit 0 | MFA/SSO deferred (`auth-mfa-sso-deferred`); E2E browser waived | Maintain |
...
| **System Admin** | `apps/erp/src/app/(protected)/system-admin/` | [`fdr-007-system-admin`](../delivery/FDR/[Partially%20Implemented]%20fdr-007-system-admin.md) Partially Implemented | **implemented** | Seven sections; mutation audit registry; `check:system-admin-mutation-audit` | §Remaining gaps in FDR; peer review (DoD #14) pending | Close remaining gaps; peer review |
| **ERP Application** | `apps/erp/` | [`fdr-007-ux-surfaces`](../delivery/FDR/[Partially%20Implemented]%20fdr-007-ux-surfaces.md) **26/30 audit · 28/30 ceiling** · PKG-007 siblings | **implemented** | TIP-UI-05 Complete; governed surfaces; CSP; ui:guard 243 files; 522/524 tests | 2 failing ERP tests (`context-switch`, `list-visible-system-admin-sections`) | Fix test failures; peer review |
...
| **Docs application (`@afenda/docs`)** | `apps/docs/` | [`fdr-005-docs-app`](../delivery/FDR/[Partially%20Implemented]%20fdr-005-docs-app.md) — Partially Implemented | **implemented** | Fumadocs + `quality:docs`; deploy config | Live DNS operator step; registry row pending | Registry-sync Slice 2 |
