...
| **CSS Authority (PAS-005)** | `@afenda/css-authority` · `packages/design-system/` token shim | `PKGR05_CSS_AUTHORITY` — green-lane (PAS-005 B26–B37 ✓) | **implemented** | 605-token registry (465 afenda + 44 appshell + 50 auth-editorial + 46 shadcn); `afenda-ui.css` → `afenda-tokens.css` + `afenda-css-authority.css`; consumption R23–R30 + domain-sync + bridge + visual contract + docs pixel baselines pass | Optional enhancements only | Maintain |
