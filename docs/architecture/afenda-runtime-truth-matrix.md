...
| **Auth** | `packages/auth/`, `apps/erp/src/app/api/auth/` | [`fdr-002-auth-disposition`](../delivery/FDR/[Complete]%20fdr-002-auth-disposition.md) **Complete — 29/30** · PKG002_AUTH · [ARCH-AUTH-001](../ARCH/[Partially%20Implemented]%20ARCH-AUTH-001-enterprise-authentication.md) slices 1–12 + 13a–13b + 13a-debt + **13c** ✓ · FR-A05.2.1 + FR-A05.2 ERP ✓ (ARCH Partially Implemented — AUTH-PHASE3-001 partial) | **implemented** | Better Auth; session→context bridge; `auth.mirror-sync` · `auth.mfa-policy` · `auth.invitation` (Postgres `member_invitations` · Slice 11); system-admin Members/Security UI (`account-settings-05/06`); admin MFA TOTP enroll/verify/disable (Slice 12); workspace session contract (`activeWorkspaceId` persisted on `auth_session` · ERP resolver merge · `AUTH_EVENT.workspaceContextSwitched` on switch); **enterprise SSO** (`@better-auth/sso` · `tenant_sso_providers` · `(tenant_id, domain)` unique index · tenant-scoped invitation gate · structured Better Auth sync results · integrations admin UI · `AUTH_EVENT.sso*` · Slice 13a + **13a-debt**); **social OAuth** (`socialProviders` Google/Microsoft · `tenant_settings.integrations.oauth` allowlist · invitation-gated sign-up · `auth.oauth-policy` · integrations admin UI · `AUTH_EVENT.oauth*` · CSP allowlist · Slice **13c**); **passkeys** (`@better-auth/passkey` · `passkey` table migration · user self-service list/register/delete on `/settings/security` · `AUTH_EVENT.passkey*` · Slice 13b); integration attestation pass (Slice 7 multiSession · Slice 13a SSO · Slice 13b passkey · Slice 13c socialOAuth extension) | Personal OAuth connections deferred (ARCH-USER-001) | Maintain; `changeEmail` for profile UI (ARCH-USER-001 cross-ref) |
...
| **Testing Infrastructure** | `packages/testing/`, `vitest.shared.ts` | [`fdr-016-test-utilities`](../delivery/FDR/[Partially%20Implemented]%20fdr-016-test-utilities.md) · [ARCH-TEST-001](../ARCH/[Partially%20Implemented]%20ARCH-TEST-001-vitest-playwright-strategy.md) — Partially Implemented | **implemented** | Shared react setup; interaction pattern; ARCH-TEST-001 pyramid authority (Slice 1) | P0 CI gates (interaction + E2E smoke) pending Slices 2–3 | ARCH-TEST-001 Slices 2–5B |
