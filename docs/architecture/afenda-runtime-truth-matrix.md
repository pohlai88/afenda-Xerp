...
| **Auth** | `packages/auth/`, `apps/erp/src/app/api/auth/` | [`fdr-002-auth-disposition`](../delivery/FDR/[Complete]%20fdr-002-auth-disposition.md) **Complete — 29/30** · PKG002_AUTH · [ARCH-AUTH-001](../ARCH/[Partially%20Implemented]%20ARCH-AUTH-001-enterprise-authentication.md) slices 1–12 + 13a–13b + 13a-debt ✓ · FR-A05.2.1 + FR-A05.2 ERP ✓ (ARCH Partially Implemented — AUTH-PHASE3-001 partial) | **implemented** | Better Auth; session→context bridge; `auth.mirror-sync` · `auth.mfa-policy` · `auth.invitation` (Postgres `member_invitations` · Slice 11); system-admin Members/Security UI (`account-settings-05/06`); admin MFA TOTP enroll/verify/disable (Slice 12); workspace session contract (`activeWorkspaceId` persisted on `auth_session` · ERP resolver merge · `AUTH_EVENT.workspaceContextSwitched` on switch); **enterprise SSO** (`@better-auth/sso` · `tenant_sso_providers` · `(tenant_id, domain)` unique index · tenant-scoped invitation gate · structured Better Auth sync results · integrations admin UI · `AUTH_EVENT.sso*` · Slice 13a + **13a-debt**); **passkeys** (`@better-auth/passkey` · `passkey` table migration · user self-service list/register/delete on `/settings/security` · `AUTH_EVENT.passkey*` · Slice 13b); integration attestation pass (Slice 7 multiSession · Slice 13a SSO · Slice 13b passkey extension) | OAuth deferred (`AUTH-PHASE3-001` slice 13c) | Maintain; `changeEmail` for profile UI (ARCH-USER-001 cross-ref) |
