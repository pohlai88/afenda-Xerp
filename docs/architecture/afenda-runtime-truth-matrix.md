...
| **CSS Authority (PAS-005)** | `@afenda/css-authority` · `packages/design-system/` token shim | `PKGR05_CSS_AUTHORITY` — green-lane (PAS-005 B26–B37 ✓) | **implemented** | 569-token registry (465 afenda + 44 appshell + 14 auth-editorial + 46 shadcn); `afenda-ui.css` → `afenda-tokens.css` + `afenda-css-authority.css`; consumption R23–R30 + domain-sync + bridge + visual contract + docs pixel baselines pass | Optional enhancements only | Maintain |
...
| **Business master data authority (Foundation phase 08)** | `packages/kernel/src/contracts/business-master-data/` · `scripts/governance/check-business-master-data-scaffold.mts` | `PKG-010 master-data-authority (pas-status-index)` **28/30 audit-adjusted** (29/30 ceiling) · Slice 2 evidence-sync ✓ (2026-06-26) · Foundation phase 08 **Complete (authority only)** | **implemented** | Frozen authority registry + wire contracts; PKG010 `quality:kernel-context-surface` exit 0; scaffold guards; 14 MD subdomain tests | PKG-R02 product/warehouse/stock runtime implemented (PKGR02 Slices 1–4) | ADR-0020 + PKGR02 Slice 3 evidence-sync |
