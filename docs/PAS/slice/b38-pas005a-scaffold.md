...
**Clean Core impact:** A→A — greenfield `packages/shadcn-studio`; legacy appshell studio untouched
...
```
Handoff from: docs/PAS/slice/b38-pas005a-scaffold.md
...
2. Allowed layer— packages/shadcn-studio/** · .cursor/mcp/shadcn-studio.mjs · shadcn-studio.config.json · docs/PAS/** · docs/architecture/package-registry.md (if PKG entry added)
3. Files        —
   ...
   docs/PAS/PAS-005A-SHADCN-STUDIO-PRESENTATION-STANDARD.md (runtime_status sync only if evidence changes)
...
7. Closes       — PAS-005A not_started → partial; MCP cwd legacy packages/ui
8. Evidence     —
   packages/shadcn-studio/package.json
   packages/shadcn-studio/src/styles/shadcn-studio.css
