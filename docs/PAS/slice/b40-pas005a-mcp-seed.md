...
**Clean Core impact:** A→A — inventory seed only; no ERP wiring

## Purpose

Seed `@afenda/shadcn-studio` via shadcn-studio MCP: `install-theme` into base CSS, `/rui` for ≥5 primitives under `src/components/ui/`, `/cui` for ≥2 blocks under `src/blocks/`. Follow collect-all-then-install rule. Document provenance headers on installed files.

## Handoff block

```
Handoff from: docs/PAS/slice/b40-pas005a-mcp-seed.md

1. Objective    — MCP-seed shadcn-studio base theme alignment, ≥5 /rui primitives, and ≥2 /cui blocks into @afenda/shadcn-studio with collection-before-install discipline and package builds cleanly.
2. Allowed layer— packages/shadcn-studio/** · shadcn-studio.config.json · docs/PAS/slice/b40-pas005a-mcp-seed.md · docs/architecture/dependency-registry.md (if new deps)
3. Files        —
...
7. Closes       — PAS-005A MCP inventory Target → Current; primitives + blocks seed evidence
8. Evidence     —
   packages/shadcn-studio/src/components/ui/ (≥5 files)
   packages/shadcn-studio/src/blocks/ (≥2 directories)
   packages/shadcn-studio/src/__tests__/mcp-seed-inventory.test.ts
...
| # | Criterion | Gate |
| --- | --- | --- |
| 1 | ≥5 primitives in `src/components/ui/` | inventory test |
| 2 | ≥2 blocks in `src/blocks/` | inventory test |
