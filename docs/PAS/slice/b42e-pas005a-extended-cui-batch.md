# Slice B42e — Extended `/cui` Batch: Account Settings + Dashboard (PAS-005A §11.4)

**Prerequisite:** B42d delivered — appshell re-export bridge + parity registry

**Status:** Delivered (2026-06-28) — account-settings 01–07 + dashboard widget/chart/datatable batch; **legacy delete still blocked**

**Type:** Implementation

**Risk class:** Medium — large MCP install (+54 files, new npm deps)

**Clean Core impact:** A→A — MCP re-seed only; no legacy TSX migration

## Handoff block

```
Handoff from: docs/PAS/slice/b42e-pas005a-extended-cui-batch.md

1. Objective    — Extended MCP /cui batch: install account-settings 01–07 + dashboard surfaces (chart, stats, datatable, widgets); expand parity registry; strict TS fixes; legacy delete remains blocked.
2. Allowed layer— packages/shadcn-studio/** · docs/PAS/slice/b42e-pas005a-extended-cui-batch.md · docs/PAS/pas-status-index.md
3. Files        —
...
7. Closes       — B42b account-settings + dashboard category MCP gap; parity registry expansion
8. Evidence     —
...
| --- | --- | --- |
| Account settings | `@ss-blocks/account-settings-01` … `-07` | `blocks/account-settings-*/` |
...
- [x] Account-settings 01–07 MCP installed
