...
| **Gates (live)** | `pnpm check:erp-module-foundation` (composite) · **12** sub-gates (ownership, knowledge, context-spine, permission, audit-outbox, metadata, database-boundary, no-kernel-runtime-leak, runtime-package-reserved, readiness, registry-readiness, procurement-runtime-foundation) |
...
| **Gates (006 family live)** | `pnpm --filter @afenda/shadcn-studio-v2 typecheck`, `pnpm --filter @afenda/shadcn-studio-v2 test:run`, `pnpm --filter @afenda/shadcn-studio-v2 build`, `pnpm --filter @afenda/shadcn-studio-v2 check:drift`, `pnpm --filter @afenda/shadcn-studio-v2 check:apca`, `pnpm check:v1-consumer-imports`, `pnpm check:downstream-integration`, `pnpm check:erp-metadata-pas006-consumer`, `pnpm --filter @afenda/erp typecheck`, `pnpm check:package-css-dist-sync`, `pnpm quality:boundaries`, `pnpm check:documentation-drift`, `pnpm check:foundation-disposition` |
