...
| 19 | `pnpm check:api-contracts` · `pnpm check:openapi-drift` | R3 | **Active** ([API-CONTRACT R3 track](../API-CONTRACT/REST/SLICE/pas-001a-r3-api-contract-runtime.md) · [PAS-API-REST-001](../API-CONTRACT/REST/PAS-API-REST-001-REST-OPENAPI-BINDING-STANDARD.md) · R3d Delivered) |
...
### 6.1.3 R3 — API contract runtime (delivered)

> **Status:** Delivered — R3a–R3d attestation closure complete (2026-06-30). IS-004 Production Accepted runtime.

| Criterion | Status | Gate evidence |
| --- | --- | --- |
| Full `createApiHandler` on all governed `/api/internal/v1/**` routes | **Green** | `pnpm check:api-contracts` ✓ |
| Registry-first OpenAPI 3.1 publication — no hand-edited JSON | **Green** | `pnpm check:openapi-drift` ✓ |
