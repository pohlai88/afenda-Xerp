...
function checkKvIdBijection(violations: ErpDomainLayoutViolation[]): void {
  const moduleSet = new Set<string>(ERP_DOMAIN_MODULES);
  const kvKeys = Object.keys(ERP_DOMAIN_MODULE_KV_IDS);
  const seenKv = new Set<string>();

  for (const slug of ERP_DOMAIN_MODULES) {
    const kvId = ERP_DOMAIN_MODULE_KV_IDS[slug as ErpDomainModule];
    if (!kvId) {
      violations.push({
        rule: "kv-id-missing-slug",
        message: `ERP_DOMAIN_MODULE_KV_IDS missing slug "${slug}".`,
      });
      continue;
    }
    if (seenKv.has(kvId)) {
      violations.push({
        rule: "kv-id-duplicate",
        message: `ERP_DOMAIN_MODULE_KV_IDS contains duplicate KV id "${kvId}".`,
      });
    }
    seenKv.add(kvId);
  }
