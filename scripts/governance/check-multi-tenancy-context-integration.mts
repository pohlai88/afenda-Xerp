#!/usr/bin/env tsx
/**
 * Multi-tenancy context integration gate (multi-tenancy.md Step 8, §572–579).
 *
 * Authoritative for protected API routes, server actions, permission feed,
 * AppShell display context, and server-side context switch validation.
 */
...
const registryPath = join(
  repoRoot,
  "scripts/governance/multi-tenancy-context-integration-registry.mts"
);
...
export type MultiTenancyContextIntegrationViolation =
...
export function checkMultiTenancyContextIntegration(): MultiTenancyContextIntegrationViolation[] {
  const violations: MultiTenancyContextIntegrationViolation[] = [];

  if (!existsSync(registryPath)) {
    violations.push({
      rule: "registry-missing",
      file: registryPath,
      message: "multi-tenancy-context-integration-registry.mts is required",
    });
    return violations;
  }
  ...
  if (!registrySource.includes(MULTI_TENANCY_CONTEXT_INTEGRATION_SURFACE_RULE)) {
    violations.push({
      ...
      file: registryPath,
      message: `Registry must export ${MULTI_TENANCY_CONTEXT_INTEGRATION_SURFACE_RULE}`,
    });
  }
  ...
  const gatePath = join(repoRoot, MULTI_TENANCY_CONTEXT_INTEGRATION_GATE);

  if (!existsSync(enforcementLibPath)) {
    violations.push({
      rule: "enforcement-lib-missing",
      file: enforcementLibPath,
      message: `${MULTI_TENANCY_CONTEXT_INTEGRATION_ENFORCEMENT_LIB} is required`,
    });
  }
  ...
  if (!existsSync(gatePath)) {
    violations.push({
      rule: "gate-missing",
      file: gatePath,
      message: `${MULTI_TENANCY_CONTEXT_INTEGRATION_GATE} is required`,
    });
  }
    ...
    for (const marker of MULTI_TENANCY_DOC_CONTEXT_INTEGRATION_MARKERS) {
      if (!multiTenancyContent.includes(marker)) {
        violations.push({
          ...
          file: multiTenancyDocPath,
          message: `Missing Step 8 marker: ${marker}`,
        });
      }
    }
  }
  ...
  if (deliveryContent === null) {
    ...
    if (
      !deliveryContent.includes(MULTI_TENANCY_CONTEXT_INTEGRATION_SURFACE_RULE)
    ) {
      violations.push({
        rule: "delivery-surface-rule-missing",
        file: deliveryDocPath,
        message: `Delivery doc must document ${MULTI_TENANCY_CONTEXT_INTEGRATION_SURFACE_RULE}`,
      });
    }

    if (
      !deliveryContent.includes(`## ${TIP_007_012_CONTEXT_INTEGRATION_SECTION}`)
    ) {
      violations.push({
  ...
  if (packageJsonContent === null) {
    ...
    if (!packageJsonContent.includes("check:multi-tenancy-context-integration")) {
      violations.push({
        ...
        file: packageJsonPath,
        message:
          "package.json must define check:multi-tenancy-context-integration",
      });
    }

    if (
      !packageJsonContent.includes("quality:multi-tenancy-context-integration")
    ) {
      violations.push({
        rule: "quality-script-missing",
        file: packageJsonPath,
        message:
          "package.json must define quality:multi-tenancy-context-integration",
      });
    }
    ...
    if (
      ...
      violations.push({
        ...
        file: packageJsonPath,
        message:
          "quality:multi-tenancy-glossary-first → existing-state-audit → authority-design → context-contracts → persistence-lookup → tenant-url-resolver → operating-context-resolver → context-integration → multi-tenancy-tests → dos-prohibitions order required in pnpm quality",
      });
    }
  }

  violations.push(...collectContextIntegrationViolations(repoRoot));

  return violations;
}

export function formatMultiTenancyContextIntegrationViolations(
  violations: readonly MultiTenancyContextIntegrationViolation[]
): string {
...
function main(): void {
  ...
  if (violations.length > 0) {
    console.error(formatMultiTenancyContextIntegrationViolations(violations));
    process.exit(1);
  }

  console.log("Multi-tenancy context integration gate passed.");
}
...
const isDirectRun = (() => {
  ...
  try {
    return (
      ...
      entry.endsWith("check-multi-tenancy-context-integration.mts")
    );
  } catch {
    return entry.endsWith("check-multi-tenancy-context-integration.mts");
  }
})();
