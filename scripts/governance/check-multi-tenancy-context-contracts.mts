#!/usr/bin/env tsx
/**
 * Multi-tenancy context contracts gate (multi-tenancy.md Step 4, §522–536).
 *
 * Authoritative for serializable kernel contracts and public export maps.
 */
...
const registryPath = join(
  repoRoot,
  "scripts/governance/multi-tenancy-context-contracts-registry.mts"
);
...
export type MultiTenancyContextContractsViolation =
...
export function checkMultiTenancyContextContracts(): MultiTenancyContextContractsViolation[] {
  const violations: MultiTenancyContextContractsViolation[] = [];

  if (!existsSync(registryPath)) {
    violations.push({
      rule: "registry-missing",
      file: registryPath,
      message: "multi-tenancy-context-contracts-registry.mts is required",
    });
    return violations;
  }

  const registrySource = readFileSync(registryPath, "utf8");
  if (!registrySource.includes(MULTI_TENANCY_CONTEXT_CONTRACTS_SURFACE_RULE)) {
    violations.push({
      rule: "registry-surface-rule-missing",
      file: registryPath,
      message: `Registry must export ${MULTI_TENANCY_CONTEXT_CONTRACTS_SURFACE_RULE}`,
    });
  }
  ...
  const gatePath = join(repoRoot, MULTI_TENANCY_CONTEXT_CONTRACTS_GATE);

  if (!existsSync(enforcementLibPath)) {
    violations.push({
      rule: "enforcement-lib-missing",
      file: enforcementLibPath,
      message: `${MULTI_TENANCY_CONTEXT_CONTRACTS_ENFORCEMENT_LIB} is required`,
    });
  }
  ...
  if (!existsSync(gatePath)) {
    violations.push({
      rule: "gate-missing",
      file: gatePath,
      message: `${MULTI_TENANCY_CONTEXT_CONTRACTS_GATE} is required`,
    });
  }
    ...
    for (const marker of MULTI_TENANCY_DOC_CONTEXT_CONTRACTS_MARKERS) {
      if (!multiTenancyContent.includes(marker)) {
        violations.push({
          ...
          file: multiTenancyDocPath,
          message: `Missing Step 4 marker: ${marker}`,
        });
      }
    }
  }
  ...
  if (deliveryContent === null) {
    ...
    if (
      ...
      violations.push({
        ...
        file: deliveryDocPath,
        message: `Delivery doc must document ${MULTI_TENANCY_CONTEXT_CONTRACTS_SURFACE_RULE}`,
      });
    }

    if (
      !deliveryContent.includes(`## ${TIP_007_012_CONTEXT_CONTRACTS_SECTION}`)
    ) {
      violations.push({
        rule: "delivery-section-missing",
        file: deliveryDocPath,
        message: `Delivery doc missing section: ## ${TIP_007_012_CONTEXT_CONTRACTS_SECTION}`,
      });
    }
  }
  ...
  if (packageJsonContent === null) {
    ...
    if (!packageJsonContent.includes("check:multi-tenancy-context-contracts")) {
      violations.push({
        rule: "check-script-missing",
        file: packageJsonPath,
        message:
          "package.json must define check:multi-tenancy-context-contracts",
      });
    }

    if (
      !packageJsonContent.includes("quality:multi-tenancy-context-contracts")
    ) {
      violations.push({
        rule: "quality-script-missing",
        file: packageJsonPath,
        message:
          "package.json must define quality:multi-tenancy-context-contracts",
      });
    }
  ...
  violations.push(...collectContextContractsViolations(repoRoot));

  return violations;
}

export function formatMultiTenancyContextContractsViolations(
  violations: readonly MultiTenancyContextContractsViolation[]
): string {
...
function main(): void {
  ...
  if (violations.length > 0) {
    console.error(formatMultiTenancyContextContractsViolations(violations));
    process.exit(1);
  }

  console.log("Multi-tenancy context contracts gate passed.");
}
...
const isDirectRun = (() => {
  ...
  try {
    return (
      ...
      entry.endsWith("check-multi-tenancy-context-contracts.mts")
    );
  } catch {
    return entry.endsWith("check-multi-tenancy-context-contracts.mts");
  }
})();
