...
export function checkMultiTenancyTenantUrlResolver(): MultiTenancyTenantUrlResolverViolation[] {
  ...
  if (deliveryContent === null) {
    ...
    if (
      !deliveryContent.includes(MULTI_TENANCY_TENANT_URL_RESOLVER_SURFACE_RULE)
    ) {
      violations.push({
        rule: "delivery-surface-rule-missing",
        file: deliveryDocPath,
        message: `Delivery doc must document ${MULTI_TENANCY_TENANT_URL_RESOLVER_SURFACE_RULE}`,
      });
    }

    if (
      !deliveryContent.includes(`## ${TIP_007_012_TENANT_URL_RESOLVER_SECTION}`)
    ) {
      violations.push({
        rule: "delivery-section-missing",
        file: deliveryDocPath,
        message: `Delivery doc missing section: ## ${TIP_007_012_TENANT_URL_RESOLVER_SECTION}`,
      });
    }
  }
