...
export function checkMultiTenancyExistingStateAudit(): MultiTenancyExistingStateAuditViolation[] {
  ...
  if (!registrySource.includes(MULTI_TENANCY_EXISTING_STATE_AUDIT_SURFACE_RULE)) {
    violations.push({
  ...
  if (deliveryContent === null) {
    ...
    if (!deliveryContent.includes(MULTI_TENANCY_EXISTING_STATE_AUDIT_SURFACE_RULE)) {
      violations.push({
        rule: "delivery-surface-rule-missing",
        file: deliveryDocPath,
        message: `Delivery doc must document ${MULTI_TENANCY_EXISTING_STATE_AUDIT_SURFACE_RULE}`,
      });
    }

    if (
      !deliveryContent.includes(`## ${TIP_007_012_EXISTING_STATE_AUDIT_SECTION}`)
    ) {
      violations.push({
        rule: "delivery-section-missing",
        file: deliveryDocPath,
        message: `Delivery doc missing section: ## ${TIP_007_012_EXISTING_STATE_AUDIT_SECTION}`,
      });
    }
  }
