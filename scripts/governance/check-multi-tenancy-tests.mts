...
export function checkMultiTenancyTests(): MultiTenancyTestsViolation[] {
  ...
  if (deliveryContent === null) {
    ...
    if (!deliveryContent.includes(MULTI_TENANCY_TESTS_SURFACE_RULE)) {
      violations.push({
        rule: "delivery-surface-rule-missing",
        file: deliveryDocPath,
        message: `Delivery doc must document ${MULTI_TENANCY_TESTS_SURFACE_RULE}`,
      });
    }

    if (!deliveryContent.includes(`## ${TIP_007_012_TESTS_SECTION}`)) {
      violations.push({
