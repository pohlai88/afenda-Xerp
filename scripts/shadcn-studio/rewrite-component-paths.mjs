...
const reps = [
  ...
  ["@/components-layouts/", "@/components/shadcn-studio/"],
  ["@/components-ui/", "@/components/ui/"],
  [
...
const roots = [
  join(repoRoot, "packages/shadcn-studio"),
...
function walk(d, f = []) {
  try {
    for (const n of readdirSync(d)) {
      if (["node_modules", "dist"].includes(n)) {
        continue;
      }
      const p = join(d, n);
      if (statSync(p).isDirectory()) {
        if (
          SKIP.some(
            (s) =>
              p.endsWith(s.replace(/\\/g, "/").slice(0, -1)) ||
              p.includes("\\components\\shadcn-studio") ||
              p.includes("/components/shadcn-studio") ||
              p.includes("\\components\\ui") ||
              p.includes("/components/ui") ||
              p.includes("\\components\\app-shell")
          )
        ) {
          continue;
        }
    ...
    // Skip unreadable directories during migration scans.
  }
  return f;
}
let c = 0;
...
for (const file of walk(
  join(repoRoot, "packages/shadcn-studio/src/components-auth-shell")
)) {
  const t = readFileSync(file, "utf8").replaceAll(
    "@/components-layouts/login-page-",
    "@/components-auth-shell/login-page-"
  );
  writeFileSync(file, t);
}
...
try {
  rmSync(join(repoRoot, "packages/shadcn-studio/src/components"), {
    recursive: true,
    force: true,
  });
...
try {
  rmSync(join(repoRoot, "packages/shadcn-studio/src/assets"), {
    recursive: true,
    force: true,
  });
