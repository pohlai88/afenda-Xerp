...
import {
  ...
  StatisticsCardSample,
  ThemeCustomizerDocumentationPreview,
  ThemeCustomizerLabSurface,
} from "../storybook/shadcn-studio-theme-lab.compositions.js";
import {
...
const meta = {
  ...
  component: ThemeCustomizerDocumentationPreview,
  tags: ["autodocs"],
...
} satisfies Meta<typeof ThemeCustomizerDocumentationPreview>;
...
export const ThemeCustomizerLab: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "ThemeCustomizer with MCP statistics-component blocks (statistics-card-01…04) for live token verification.",
      },
    },
  },
  play: async ({ canvas }) => {
    await expect(
      canvas.getByRole("region", { name: "Theme customizer" })
    ).toBeVisible();
...
export const StatisticsCardPreview: Story = {
  render: () => <StatisticsCardSample />,
  parameters: {
    docs: {
      description: {
        story: "Single statistics-card-01 block — MCP statistics-component-01.",
      },
    },
  },
};
