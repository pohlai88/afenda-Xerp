...
const BLOCK_ID = "error-access-denied-page-01" as const;
...
export default function ErrorAccessDeniedPage01() {
  return (
    <AuthShellStage
      {...blockSlotDomMarkerProps("error-access-denied-page-01.content")}
      variant="error"
    >
      <Card className="w-full max-w-md gap-6 py-6">
        <CardHeader className="gap-5 px-6 text-center">
          ...
          <div className="space-y-2">
            <CardTitle
              {...blockSlotDomMarkerProps("error-access-denied.title")}
              className="font-semibold text-2xl"
            >
              Access denied
            </CardTitle>
            <CardDescription
              {...blockSlotDomMarkerProps("error-access-denied.message")}
              className="text-base"
            >
              This workspace requires an approved account or a fresh sign in.
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent className="px-6">
          <div aria-hidden="true" className="sr-only">
            <span {...blockSlotDomMarkerProps("error-access-denied.cta")} />
          </div>
          <Button
            className="w-full"
            nativeButton={false}
