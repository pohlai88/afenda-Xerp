...
const BLOCK_ID = "invite-consumed-page-01" as const;
...
export default function InviteConsumedPage01() {
  return (
    <AuthShellStage
      {...blockSlotDomMarkerProps("invite-consumed-page-01.content")}
      variant="invite"
    >
      <Card className="w-full max-w-md gap-6 py-6">
        <CardHeader className="gap-5 px-6 text-center">
          ...
          <div className="space-y-2">
            <CardTitle
              {...blockSlotDomMarkerProps("invite-consumed.title")}
              className="font-semibold text-2xl"
            >
              Invitation already used
            </CardTitle>
            <CardDescription
              {...blockSlotDomMarkerProps("invite-consumed.message")}
              className="text-base"
            >
              This invitation has already been consumed. Sign in with the
        ...
        <CardContent className="space-y-3 px-6">
          <div aria-hidden="true" className="sr-only">
            <span {...blockSlotDomMarkerProps("invite-consumed.cta")} />
            <span {...blockSlotDomMarkerProps("invite-consumed.back")} />
          </div>
          <Button
            className="w-full"
            nativeButton={false}
