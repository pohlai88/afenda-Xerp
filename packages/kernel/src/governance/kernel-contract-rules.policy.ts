...
export const KERNEL_CONTRACT_RULES_POLICY = {
  pasSection: "9",
  ruleCount: KERNEL_CONTRACT_RULES.length,
  delegatedGateIds: [
...
export function getKernelContractRule(
  ...
  const rule = KERNEL_CONTRACT_RULES.find((entry) => entry.id === id);
  if (rule === undefined) {
    throw new Error(`Unknown kernel contract rule id: ${id}`);
  }
  return rule;
}

export function listKernelContractRules(): readonly KernelContractRule[] {
  return KERNEL_CONTRACT_RULES;
}

export function getKernelContractRuleByPasNumber(
  pasRuleNumber: number
): KernelContractRule | undefined {
  return KERNEL_CONTRACT_RULES.find(
