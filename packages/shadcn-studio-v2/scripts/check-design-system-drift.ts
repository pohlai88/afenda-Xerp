...
const QUARANTINE_ROOT = path.join(PACKAGE_SRC, "components", "quarantine");
const QUARANTINE_INVENTORY_BASELINE_PATH = path.join(
  QUARANTINE_ROOT,
...
const QUARANTINE_STALE_ADVISORY_DAYS = 30;
...
interface QuarantineInventorySummary {
...
const checkQuarantineGovernance = async (): Promise<{
  readonly summary: QuarantineInventorySummary;
  readonly violations: DriftViolation[];
}> => {
