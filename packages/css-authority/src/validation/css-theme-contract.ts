...
function validateUiImportChain(
  ...
  if (uiCss.includes("@afenda/design-system/")) {
    issues.push({
      code: "ui-design-system-import",
      message: "afenda-ui.css must not import @afenda/design-system (retired)",
    });
  }
}
...
function validateCssAuthorityTokens(
  ...
  if (!tokens.includes("--afenda-")) {
    issues.push({
      ...
      message:
        "css-authority afenda-tokens.css must define --afenda-* variables",
    });
  }
}
