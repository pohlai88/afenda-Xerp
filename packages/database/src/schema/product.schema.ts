...
export const PRODUCTS_TENANT_SKU_UNIQUE_INDEX =
  tenantHumanReferenceUniqueIndexName("products", "sku");
export const PRODUCTS_ENTERPRISE_ID_UNIQUE_INDEX =
  enterpriseIdUniqueIndexName("products");

export const products = pgTable(
  "products",
  {
    id: primaryId(),
    enterpriseId: enterpriseIdColumn("product"),
    tenantId: tenantIdRef()
      .notNull()
      .references(() => tenants.id, { onDelete: "restrict" }),
    sku: tenantHumanReferenceColumn("sku"),
  ...
  (table) => [
    ...
    uniqueIndex(PRODUCTS_ENTERPRISE_ID_UNIQUE_INDEX).on(table.enterpriseId),
    enterpriseIdFormatCheck(table.enterpriseId, "product"),
    index(tenantForeignKeyIndexName("products")).on(table.tenantId),
