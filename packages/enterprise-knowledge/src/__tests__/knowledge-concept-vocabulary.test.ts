...
describe("PAS-004C §4.1 — concept + vocabulary layer (B38)", () => {
  ...
  it("all 44 atoms carry conceptId resolving to concepts.json", () => {
    const conceptsRaw = JSON.parse(
      readFileSync(join(dataDir, "concepts.json"), "utf8")
    ) as unknown[];
    ...
    expect(atomsRaw).toHaveLength(44);
  ...
  it("public exports include concept and term corpora", () => {
    expect(ENTERPRISE_KNOWLEDGE_CONCEPTS.length).toBe(44);
    expect(ENTERPRISE_KNOWLEDGE_TERMS.length).toBe(44);
